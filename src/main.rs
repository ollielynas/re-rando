...
impl Table {
    fn new(keys: Vec<String>, values: Vec<String>) -> Self {
        ...
        new.mean_diff = (
            new.group1.iter().sum::<f32>() / (new.group1.len() as f32) -
            new.group2.iter().sum::<f32>() / (new.group2.len() as f32)
        ).abs();
    ...
    fn re_randomize(&mut self) {
        ...
        for i in 0..self.key.len() {
            ...
            } else if self.median_check && self.meds.1 == 0.0 {
        ...
        if
            (
                (self.group1_total / (self.group1.len() as f32)) -
                ((self.sum_total - self.group1_total) / (self.group2.len() as f32))
            ).abs() >= self.mean_diff
        {
            self.mean_total += 1;
        }
...
fn main() {
    ...
    sycamore::render(|cx| {
        ...
        let saves = match
            (<LocalStorage as Storage>::get("input1"), <LocalStorage as Storage>::get("input2"))
        {
            (Ok(a), Ok(b)) => (a, b),
        ...
        let mean_check = create_signal(cx, true);
        let median_check = create_signal(cx, true);
        ...
        create_effect(cx, || {
            ...
            let values = in2
                ...
                .map(|s| s.to_string())
                .collect::<Vec<String>>();
            let mut t = Table::new(keys, values);
            t.mean_check = *mean_check.get();
            t.median_check = *median_check.get();
            table.set(t);
        ...
        div (class="") {
            ...
            div (class="group") {
            div (class="tick") {
                input (type="checkbox",id="c1", title="mean", bind:checked=mean_check) {("mean")}
                label(for="c1") {("mean")}
            }
            div (class="tick") {
                input (type="checkbox",id="c2", bind:checked=median_check) {("median")}
                label(for="c2") {("median")}
            }
            }
